...
class FsUIClient:
    def __init__(self, ip, port, cname, get_participant, *args):
        print "New FsUIClient initilising"
        self.participants = {}
        ...
        print "New FsUIClient Initialised"
    ...
    def __intro(self, src, dest, media, cname):
        print "Got Intro from %s here" % src
        if src == 1:
            self.connect.myid = dest
        if not self.participants.has_key(src):
            if src != 1:
                self.connect.send_intro(src, self.cname)
            test =  self.get_participant(self.connect, src,
                                                          cname,
                                                          *self.args)   
            self.participants[src] = test
        print "Going out of intro"
...
if __name__ == "__main__":
    ...
    class TestParticipant:
        def __init__(self, connect, id, cname, *args):
            print "Preparing New Participant %s and cname %s" % (id,cname)
            self.id = id

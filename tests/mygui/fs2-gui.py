...
class FsUIPipeline:
    ...
    def __init__(self, elementname="fsmsnconference"):
        ...
        sourcepad = self.videosource.get_src_pad()
        sourcepad.link(sink_pad)
        
        self.pipeline.set_state(gst.STATE_PLAYING)
        print "Pipeline is playing, all should be good"
...
class FsUISession:
    ...
    def new_stream(self, id, participant, direction):
        ...
        if VIDEO:
            self.streams[int(farsight.MEDIA_TYPE_VIDEO)] = \
              pipeline.videosession.new_stream(
                int(farsight.MEDIA_TYPE_VIDEO), self,farsight.DIRECTION_RECV)
...
class FsUIParticipant2:
    ...
    def __init__(self, connect, id, cname, pipeline, mainui):
        print "New Participant being created"
        self.connect = connect
        self.id = id
        self.cname = cname
        self.pipeline = pipeline
        ...
        if VIDEO:
            self.streams[int(farsight.MEDIA_TYPE_VIDEO)] = \
              pipeline.videosession.new_stream(
                int(farsight.MEDIA_TYPE_VIDEO), self,farsight.DIRECTION_SEND)
    ...
    def __init__(self, mode, ip, port):
        ...
        if mode == CLIENT:
            ...
            self.server = FsUIListener(port, FsUIServer, mycname,
                                       FsUIParticipant2, self.pipeline, self)
            self.glade.get_widget("info_label").set_markup(
